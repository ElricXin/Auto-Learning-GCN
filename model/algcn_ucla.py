...
class ALSGCN(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups=8, coff_embedding=4, num_subset=3,t_stride=1,t_padding=0,t_dilation=1,bias=True):
        ...
        self.num_subset = 3
        self.GroupA = nn.Parameter(torch.tensor(np.reshape(A.astype(np.float32),[3,1,20,20]), dtype=torch.float32, requires_grad=True).repeat(1,groups,1,1), requires_grad=True)
        self.GroupMA = nn.Parameter(torch.tensor(np.reshape(A.astype(np.float32),[3,1,20,20]), dtype=torch.float32, requires_grad=True).repeat(1,groups,1,1), requires_grad=True)
        self.A = Variable(torch.from_numpy(np.reshape(A.astype(np.float32),[3,1,20,20]).repeat(groups,axis=1)), requires_grad=False)
        ...
        self.fc = nn.Linear(50, 20)
        nn.init.normal(self.fc.weight, 0, math.sqrt(2. / 20))
    ...
    def forward(self, x):
        ...
        m=self.conv(m)
        n, kc, t, v = m.size()
        A_final=torch.zeros([N,self.num_subset,self.out_channels,20,20],dtype=torch.float,device='cuda').detach()
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=20, num_person=2, graph=None, graph_7=None, graph_args=dict(), in_channels=3):
        ...
        self.l1 = TCN_GCN_unit(80, 80, A, 52, residual=False)
        self.l2 = TCN_GCN_unit(80, 80, A, 52)
        self.l3 = TCN_GCN_unit(80, 80, A, 52)
        self.l4 = TCN_GCN_unit(80, 80, A, 52)
        self.l5 = TCN_GCN_unit(80, 160, A, 26, stride=2)
        self.l6 = TCN_GCN_unit(160, 160, A, 26)
        self.l7 = TCN_GCN_unit(160, 160, A, 26)
        self.l8 = TCN_GCN_unit(160, 320, A, 13, stride=2)
        self.l9 = TCN_GCN_unit(320, 320, A, 13)
        self.l10 = TCN_GCN_unit(320, 320, A, 13)
